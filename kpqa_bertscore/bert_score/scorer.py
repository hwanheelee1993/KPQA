...
class BERTScorer:
    ...
    def __init__(
        ...
        lang=None, rescale_with_baseline=False, stopwords=None,model_path=None,
    ):
        """
        Args:
        ...
        self._model = get_model(self.model_type, self.num_layers, self.all_layers, self.model_path)
    ...
    def compute_idf(self, sents):
        ...
        max_idf = max([self._idf_dict[x] for x in self._idf_dict])

        '''
        for x in self._idf_dict:
            self._idf_dict[x] = self._idf_dict[x]/max_idf        
        '''
        
       
        
    def score(self, cands, refs, w_hyps=None, w_refs=None, verbose=False, batch_size=64, return_hash=False):
        """
        Args:
